...
import uuid
from langgraph_v21.graph_build import build, prepare_state
import ollama
import traceback
...
st.title("GraphForge v23 Dashboard 🧠")
...
if st.button("📝 対話内容を .md として下に反映"):
    md_text = "\n\n".join([f"{m['role'].upper()}: {m['content']}" for m in st.session_state.chat_history if m['role'] in ("user", "assistant")])
